...
export type MortgageErrors = Partial<Record<
  'propertyValue' | 'deposit' | 'depositTooBig' | 'occupancy' | 'propertyType',
  string
>>;
...
function validate(state: MortgageState): MortgageErrors {
  ...
  if (state.propertyValue && state.deposit && state.deposit > state.propertyValue)
    e.depositTooBig = 'Deposit cannot exceed property value.';
  if (!state.occupancy) e.occupancy = 'Select Owner-Occupied or Investment.';
  if (!state.propertyType) e.propertyType = 'Select Brand New, Existing or Land.';
  return e;
}
...
export function MortgageCalculatorProvider({ children }: { children: React.ReactNode }) {
  ...
  const value: CtxValue = useMemo(
    () => ({
      ...
      setDepositMenuOpen: (b) => dispatch({ type: 'setDepositMenuOpen', value: b }),
      ...
      submit: () => {
        dispatch({ type: 'touch' });
        if (Object.keys(validate({ ...state, touched: true })).length > 0) return;
...
export function useMortgageCalculator() {
  ...
  if (!ctx) throw new Error('useMortgageCalculator must be used within MortgageCalculatorProvider');
  return ctx;
}
